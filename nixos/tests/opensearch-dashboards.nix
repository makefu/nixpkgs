  ...
  { pkgs, lib, ... }:
  {
    ...
    nodes.machine = {
      ...
      services.opensearch-dashboards = {
        enable = true;
        package = pkgs.opensearch-dashboards.overrideAttrs (old: {
          # Remove the security dashboards plugin since OpenSearch
          # security is disabled; it would block all API requests with 401
          installPhase =
            old.installPhase
            + ''
