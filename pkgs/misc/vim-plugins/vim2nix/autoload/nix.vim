...
fun! nix#AddNixDerivation(opts, cache, name, ...) abort
  ...
  if repository == {}
    ...
    if len(list) > 0
      if '' != list[2]
        let name = list[2]
        let repository = { 'type': 'git', 'owner': list[1], 'repo': list[2], 'url': 'http://github.com/'.list[1].'/'.list[2] }
      else
        let name = list[1]
        let repository = { 'type': 'git', 'owner': list[1], 'repo': 'vim-addon-'.list[1], 'url': 'http://github.com/'.list[1].'/vim-addon-'.list[1] }
      endif
    else
